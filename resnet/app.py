from resnet.models import object_detection
...
def demo(mode=CAMERA_MODE,imgfile='resnet/test_images/4.jpg',imgfilename='noname'):
    if mode == STATIC_MODE:
        img_fp = imgfile#'resnet/test_images/4.jpg'
        img = cv2.imread(img_fp)
        print('image is:')
        print(img_fp)
        net.predict(img=img, display_img=img,filename=imgfilename)
        # cv2.waitKey()
